...
V2_STABLE_DIFFUSION_PATH = "stabilityai/stable-diffusion-2"  # tokenizer v2v2.1tokenizer
...
class BucketManager:
    ...
    def select_bucket(self, image_width, image_height):
        ...
        if not self.no_upscale:
            # 
            # aspect ratio（fine tuningno_upscale=True）
    ...
    def get_crop_ltrb(bucket_reso: Tuple[int, int], image_size: Tuple[int, int]):
        # Stability AIcrop left/topcrop rightflipaugmentation
...
class BaseSubset:
    def __init__(
        ...
        self.token_warmup_step = token_warmup_step  # N（N<1N*max_train_steps）
...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def make_buckets(self):
        ...
        # bucketbucket
        if self.enable_bucket:
            if self.bucket_manager is None:  # fine tuningmetadata
                ...
                if not self.bucket_no_upscale:
                    ...
                    print(
                        "min_bucket_reso and max_bucket_reso are ignored if bucket_no_upscale is set, because bucket reso is defined by image size automatically / bucket_no_upscalebucketmin_bucket_resomax_bucket_reso"
                    )
    ...
    # weight_dtypeText Encoderweight_dtype
    # SDXLdatasetsdxl_train_util.py
    # SD1/2v2
    ...
    def crop_target(self, subset: BaseSubset, image, face_cx, face_cy, face_w, face_h):
        ...
        if min_scale >= max_scale:  # rangemin==max
            scale = min_scale
        else:
    ...
    def __getitem__(self, index):
        ...
        for image_key in bucket[image_index : image_index + bucket_batch_size]:
            ...
            if image_info.latents is not None:  # cache_latents=True
                ...
                image = None
            elif image_info.latents_npz is not None:  # FineTuningDatasetcache_latents_to_disk=True
                ...
                if self.enable_bucket:
                    ...
                    if face_cx > 0:  # 
                        ...
                        assert (
                            subset.random_crop
                        ), f"image too large, but cropping and bucketing are disabled / face_crop_aug_rangerandom_cropbucket: {image_info.absolute_path}"
                        if im_h > self.height:
                ...
                latents = None
                image = self.image_transforms(img)  # -1.0~1.0torch.Tensor
...
class DreamBoothDataset(BaseDataset):
    def __init__(
        ...
        if self.enable_bucket:
            ...
            ), f"min_bucket_reso must be equal or less than resolution / min_bucket_resomin_bucket_reso"
            assert (
                max(resolution) <= max_bucket_reso
            ), f"max_bucket_reso must be equal or greater than resolution / max_bucket_resomin_bucket_reso"
        ...
        for subset in subsets:
            if subset.num_repeats < 1:
                print(
                    f"ignore subset with image_dir='{subset.image_dir}': num_repeats is less than 1 / num_repeats1: {subset.num_repeats}"
                )
                continue
...
class FineTuningDataset(BaseDataset):
    def __init__(
        ...
        for subset in subsets:
            if subset.num_repeats < 1:
                print(
                    f"ignore subset with metadata_file='{subset.metadata_file}': num_repeats is less than 1 / num_repeats1: {subset.num_repeats}"
                )
                continue
        ...
        if use_npz_latents:
            ...
            if not npz_any:
                ...
                print(f"npz file does not exist. ignore npz files / npznpz")
            elif not npz_all:
                use_npz_latents = False
                print(f"some of npz file does not exist. ignore npz files / npznpz")
                if flip_aug_in_subset:
                    print("maybe no flipped files / npz")
        # else:
        #   print("npz files are not used with color_aug and/or random_crop / color_augrandom_cropnpz")
        ...
        if sizes is None:
            if use_npz_latents:
                use_npz_latents = False
                print(f"npz files exist, but no bucket info in metadata. ignore npz files / bucketnpz")

            assert (
                resolution is not None
            ), "if metadata doesn't have bucket info, resolution is required / bucketresolution"
            ...
            if not enable_bucket:
                print("metadata has bucket info, enable bucketing / bucketbucket")
            print("using bucket info in metadata / bucket")
            self.enable_bucket = True

            assert (
                not bucket_no_upscale
            ), "if metadata has bucket info, bucket reso is precalculated, so bucket_no_upscale cannot be used / bucketbucketbucket_no_upscale"
    ...
    def __getitem__(self, index):
        ...
        for i, image_key in enumerate(bucket[image_index : image_index + bucket_batch_size]):
            ...
            if self.dreambooth_dataset_delegate.enable_bucket:
                cond_img = cv2.resize(cond_img, image_info.resized_size, interpolation=cv2.INTER_AREA)  # INTER_AREAcv2
                assert (
...
def is_disk_cached_latents_is_expected(reso, npz_path: str, flip_aug: bool):
    expected_latents_size = (reso[1] // 8, reso[0] // 8)  # bucket_resoWxH
...
def debug_dataset(train_dataset, show_input_ids=False):
    ...
    print("`S` for next step, `E` for next epoch no. , Escape for exit. / SEEsc")
...
def trim_and_resize_if_required(
    ...
    if image_width != resized_size[0] or image_height != resized_size[1]:
        # 
        image = cv2.resize(image, resized_size, interpolation=cv2.INTER_AREA)  # INTER_AREAcv2
...
def replace_vae_modules(vae: diffusers.models.AutoencoderKL, mem_eff_attn, xformers):
    ...
    if mem_eff_attn:
        ...
        # DiffusersxformersAttentionMidBlock
...
def add_sd_models_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="pretrained model to train, directory to Diffusers model or StableDiffusion checkpoint / DiffusersStableDiffusionckpt",
    )
    parser.add_argument(
        "--tokenizer_cache_dir",
        type=str,
...
def add_optimizer_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--max_grad_norm", default=1.0, type=float, help="Max gradient norm, 0 for no clipping / norm0clipping"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1,
        help="Polynomial power for polynomial scheduler / polynomialpolynomial power",
    )
...
def add_training_arguments(parser: argparse.ArgumentParser, support_dreambooth: bool):
    ...
    parser.add_argument(
        "--save_state_to_huggingface", action="store_true", help="save state to huggingface / huggingfacestate"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="save checkpoint N epoch ratio (for example 5 means save at least 5 files total) / （55）",
    )
    parser.add_argument(
        "--save_last_n_epochs",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="save last N checkpoints of state (overrides the value of --save_last_n_epochs)/ Nstate（--save_last_n_epochs）",
    )
    parser.add_argument(
        "--save_last_n_steps",
        type=int,
    ...
    parser.add_argument(
        "--save_state",
        action="store_true",
        help="save training state additionally (including optimizer states etc.) / optimizerstate",
    )
    ...
    parser.add_argument(
        ...
        help="max token length of text encoder (default for 75, 150 or 225) / text encoder（75150225）",
    )
    parser.add_argument(
        "--mem_eff_attn",
        action="store_true",
        help="use memory efficient attention for CrossAttention / CrossAttentionattention",
    )
    parser.add_argument("--xformers", action="store_true", help="use xformers for CrossAttention / CrossAttentionxformers")
    parser.add_argument(
        "--sdpa",
        action="store_true",
        help="use sdpa for CrossAttention (requires PyTorch 2.0) / CrossAttentionsdpa（PyTorch 2.0）",
    )
    parser.add_argument(
        "--vae", type=str, default=None, help="path to checkpoint of vae to replace / VAEVAEcheckpoint"
    )
    ...
    parser.add_argument(
        "--full_bf16", action="store_true", help="bf16 training including gradients / bf16"
    )  # TODO move to SDXL training, because it is not supported by SD1/2
    parser.add_argument(
        "--clip_skip",
        type=int,
        default=None,
        help="use output of nth layer from back of text encoder (n>=1) / text encodern（n1）",
    )
    parser.add_argument(
        "--logging_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        help="what logging tool(s) to use (if 'all', TensorBoard and WandB are both used) /  (allTensorBoardWandB)",
    )
    ...
    #     type=int,
    #     default=None,
    #     help="enable perlin noise and set the octaves / perlin noiseoctaves",
    # )
    parser.add_argument(
        "--multires_noise_discount",
        type=float,
        default=0.3,
        help="set discount value for multires noise (has no effect without --multires_noise_iterations) / Multires noisediscount（--multires_noise_iterations）",
    )
    parser.add_argument(
        "--adaptive_noise_scale",
        type=float,
        default=None,
        help="add `latent mean absolute value * this value` to noise_offset (disabled if None, default) / latent * noise_offset（None）",
    )
    parser.add_argument(
        "--zero_terminal_snr",
        action="store_true",
        help="fix noise scheduler betas to enforce zero terminal SNR / noise schedulerbetaszero terminal SNR",
    )
    parser.add_argument(
        "--min_timestep",
        type=int,
        default=None,
        help="set minimum time step for U-Net training (0~999, default is 0) / U-Nettime step（0~999(0)） ",
    )
    parser.add_argument(
        "--max_timestep",
        type=int,
        default=None,
        help="set maximum time step for U-Net training (1~1000, default is 1000) / U-Nettime step（1~1000(1000)）",
    )

    parser.add_argument(
        "--lowram",
        action="store_true",
        help="enable low RAM optimization. e.g. load models to VRAM instead of RAM (for machines which have bigger VRAM than RAM such as Colab and Kaggle) / VRAM（ColabKaggleRAMVRAM）",
    )
...
def verify_training_args(args: argparse.Namespace):
    if args.v_parameterization and not args.v2:
        print("v_parameterization should be with v2 not v1 or sdxl / v1sdxlv_parameterization")
    if args.v2 and args.clip_skip is not None:
        print("v2 with clip_skip will be unexpected / v2clip_skip")

    if args.cache_latents_to_disk and not args.cache_latents:
        args.cache_latents = True
        print(
            "cache_latents_to_disk is enabled, so cache_latents is also enabled / cache_latents_to_diskcache_latents"
        )
    ...
    if args.noise_offset is not None and args.multires_noise_iterations is not None:
        raise ValueError(
            "noise_offset and multires_noise_iterations cannot be enabled at the same time / noise_offsetmultires_noise_iterations"
        )
    # if args.noise_offset is not None and args.perlin_noise is not None:
    #     raise ValueError("noise_offset and perlin_noise cannot be enabled at the same time / noise_offsetperlin_noise")
    # if args.perlin_noise is not None and args.multires_noise_iterations is not None:
    #     raise ValueError(
    #         "perlin_noise and multires_noise_iterations cannot be enabled at the same time / perlin_noisemultires_noise_iterations"
    #     )

    if args.adaptive_noise_scale is not None and args.noise_offset is None:
        raise ValueError("adaptive_noise_scale requires noise_offset / adaptive_noise_scalenoise_offset")

    if args.scale_v_pred_loss_like_noise_pred and not args.v_parameterization:
        raise ValueError(
            "scale_v_pred_loss_like_noise_pred can be enabled only with v_parameterization / scale_v_pred_loss_like_noise_predv_parameterization"
        )
    
    if args.v_pred_like_loss and args.v_parameterization:
        raise ValueError(
            "v_pred_like_loss cannot be enabled with v_parameterization / v_pred_like_lossv_parameterization"
        )
    ...
    if args.zero_terminal_snr and not args.v_parameterization:
        print(
            ...
            + " / zero_terminal_snrv_parameterization"
        )
...
def add_dataset_arguments(
    ...
    parser.add_argument(
        "--shuffle_caption", action="store_true", help="shuffle comma-separated caption / captionshuffle"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--cache_latents",
        action="store_true",
        help="cache latents to main memory to reduce VRAM usage (augmentations must be disabled) / VRAMlatentcache（augmentation） ",
    )
    parser.add_argument("--vae_batch_size", type=int, default=1, help="batch size for caching latents / latentcache")
    parser.add_argument(
        "--cache_latents_to_disk",
        action="store_true",
        help="cache latents to disk to reduce VRAM usage (augmentations must be disabled) / VRAMlatentcache（augmentation）",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="steps of resolution for buckets, divisible by 8 is recommended / bucket8",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0,
        help="tag length reaches maximum on N steps (or N*max_train_steps if N<1) / N（N<1N*max_train_steps）0（）",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="dataset class for arbitrary dataset (package.module.Class) /  (package.module.Class)",
    )

    if support_caption_dropout:
        # Textual Inversion captiondropoutsupport
        # tensorDropoutprefixcaptionevery_n_epochsdefault None
        parser.add_argument(
            "--caption_dropout_rate", type=float, default=0.0, help="Rate out dropout caption(0.0~1.0) / captiondropout"
        )
        parser.add_argument(
            "--caption_dropout_every_n_epochs",
            type=int,
            default=0,
            help="Dropout all captions every N epochs / captiondropout",
        )
        parser.add_argument(
            "--caption_tag_dropout_rate",
            type=float,
    ...
    if support_caption:
        # caption dataset
        parser.add_argument("--in_json", type=str, default=None, help="json metadata for dataset / metadatajson")
        parser.add_argument(
...
def add_sd_saving_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--use_safetensors",
        action="store_true",
        help="use safetensors format to save (if save_model_as is not specified) / checkpointsafetensors（save_model_as）",
    )
...
def get_optimizer(args, trainable_params):
    ...
    if args.use_8bit_adam:
        ...
        ), "both option use_8bit_adam and use_lion_optimizer are specified / use_8bit_adamuse_lion_optimizer"
        assert (
            optimizer_type is None or optimizer_type == ""
        ), "both option use_8bit_adam and optimizer_type are specified / use_8bit_adamoptimizer_type"
        optimizer_type = "AdamW8bit"

    elif args.use_lion_optimizer:
        assert (
            optimizer_type is None or optimizer_type == ""
        ), "both option use_lion_optimizer and optimizer_type are specified / use_lion_optimizeroptimizer_type"
        optimizer_type = "Lion"
    ...
    if optimizer_type == "Lion".lower():
        ...
        if optimizer_type == "AdamW8bit".lower():
            ...
            if "momentum" not in optimizer_kwargs:
                print(
                    f"8-bit SGD with Nesterov must be with momentum, set momentum to 0.9 / 8-bit SGD with Nesterovmomentum0.9"
                )
            ...
            try:
                ...
                raise AttributeError(
                    "No Lion8bit. The version of bitsandbytes installed seems to be old. Please install 0.38.0 or later. / Lion8bitbitsandbytes0.38.0"
                )
            ...
            try:
                ...
                raise AttributeError(
                    "No PagedAdamW8bit. The version of bitsandbytes installed seems to be old. Please install 0.39.0 or later. / PagedAdamW8bitbitsandbytes0.39.0"
                )
            ...
            try:
                ...
                raise AttributeError(
                    "No PagedLion8bit. The version of bitsandbytes installed seems to be old. Please install 0.39.0 or later. / PagedLion8bitbitsandbytes0.39.0"
                )
        ...
        if "momentum" not in optimizer_kwargs:
            print(f"SGD with Nesterov must be with momentum, set momentum to 0.9 / SGD with Nesterovmomentum0.9")
            optimizer_kwargs["momentum"] = 0.9
        ...
        if actual_lr <= 0.1:
            print(
                f"learning rate is too low. If using D-Adaptation or Prodigy, set learning rate around 1.0 / D-AdaptationProdigy1.0: lr={actual_lr}"
            )
            print("recommend option: lr=1.0 / 1.0")
        if lr_count > 1:
            print(
                f"when multiple learning rates are specified with dadaptation (e.g. for Text Encoder and U-Net), only the first one will take effect / D-AdaptationProdigy（Text EncoderU-Net）: lr={actual_lr}"
            )
        ...
        if optimizer_type.startswith("DAdapt".lower()):
            ...
            try:
                ...
                raise ImportError("No Prodigy / Prodigy ")
        ...
        if not optimizer_kwargs["relative_step"] and optimizer_kwargs.get("warmup_init", False):
            print(f"set relative_step to True because warmup_init is True / warmup_initTruerelative_stepTrue")
            optimizer_kwargs["relative_step"] = True
        print(f"use Adafactor optimizer | {optimizer_kwargs}")

        if optimizer_kwargs["relative_step"]:
            print(f"relative_step is true / relative_steptrue")
            if lr != 0.0:
                print(f"learning rate is used as initial_lr / learning rateinitial_lr")
            args.learning_rate = None

            # trainable_paramsgroup：lr
            if type(trainable_params) == list and type(trainable_params[0]) == dict:
                has_group_lr = False
                for group in trainable_params:
                    p = group.pop("lr", None)
                ...
                if has_group_lr:
                    # args TODO 
                    print(f"unet_lr and text_encoder_lr are ignored / unet_lrtext_encoder_lr")
                    args.unet_lr = None
            ...
            if args.max_grad_norm != 0.0:
                print(
                    f"because max_grad_norm is set, clip_grad_norm is enabled. consider set to 0 / max_grad_normclip_grad_norm0"
                )
            if args.lr_scheduler != "constant_with_warmup":
                print(f"constant_with_warmup will be good / constant_with_warmup")
            if optimizer_kwargs.get("clip_threshold", 1.0) != 1.0:
                print(f"clip_threshold=1.0 will be good / clip_threshold1.0")
...
def get_scheduler_fix(args, optimizer: Optimizer, num_processes: int):
    ...
    if name.startswith("adafactor"):
        ...
        ), f"adafactor scheduler must be used with Adafactor optimizer / adafactor schedulerAdafactor"
...
def prepare_dataset_args(args: argparse.Namespace, support_metadata: bool):
    ...
    if support_metadata:
        if args.in_json is not None and (args.color_aug or args.random_crop):
            print(
                f"latents in npz is ignored when color_aug or random_crop is True / color_augrandom_cropnpzlatents"
            )
    ...
    if args.log_with is None:
        ...
        if log_with in ["tensorboard", "all"]:
            if logging_dir is None:
                raise ValueError("logging_dir is required when log_with is tensorboard / Tensorboardlogging_dir")
...
def get_hidden_states_sdxl(
    ...
    if max_token_length is not None:
        # bs*3, 77, 768 or 1024
        # encoder1: <BOS>...<EOS>  <BOS>...<EOS> 
        states_list = [hidden_states1[:, 0].unsqueeze(1)]  # <BOS>
        for i in range(1, max_token_length, tokenizer1.model_max_length):
            states_list.append(hidden_states1[:, i : i + tokenizer1.model_max_length - 2])  # <BOS>  <EOS> 
        ...
        for i in range(1, max_token_length, tokenizer2.model_max_length):
            chunk = hidden_states2[:, i : i + tokenizer2.model_max_length - 2]  # <BOS>  
            # this causes an error:
            # RuntimeError: one of the variables needed for gradient computation has been modified by an inplace operation
            # if i > 1:
            #     for j in range(len(chunk)):  # batch_size
            #         if input_ids2[n_index + j * n_size, 1] == tokenizer2.eos_token_id:  #  <BOS> <EOS> <PAD> ...
            #             chunk[j, 0] = chunk[j, 1]  #  <PAD> 
            states_list.append(chunk)  # <BOS>  <EOS> 
        states_list.append(hidden_states2[:, -1].unsqueeze(1))  # <EOS>  <PAD> 
...
def get_remove_step_no(args: argparse.Namespace, step_no: int):
    ...
    # last_n_stepsstep_nosave_every_n_stepsstep_no
    # save_every_n_steps=10, save_last_n_steps=3050step30step10step
...
# epochstepepoch/step
# on_epoch_end: TrueepochFalsestep
...
def sample_images_common(
    ...
    """
    StableDiffusionLongPromptWeightingPipelineclip skip
    """
    ...
    elif args.sample_sampler == "ddpm":  # ddpmoption
...
# collate_fn epoch,stepmultiprocessing.Value
