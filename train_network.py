...
class NetworkTrainer:
    ...
    def train(self, args):
        ...
        if args.dataset_class is None:
            ...
            if use_user_config:
                ...
                if any(getattr(args, attr) is not None for attr in ignored):
                    print(
                        "ignoring the following options because config file is found: {0} / : {0}".format(
                            ", ".join(ignored)
                        )
                    )
            else:
        ...
        if len(train_dataset_group) == 0:
            print(
                "No data found. Please verify arguments (train_data_dir must be the parent of folders with images) / （train_data_dir）"
            )
            return
        ...
        if cache_latents:
            ...
            ), "when caching latents, either color_aug or random_crop cannot be used / latentcolor_augrandom_crop"
        ...
        if args.scale_weight_norms and not hasattr(network, "apply_max_norm_regularization"):
            print(
                "warning: scale_weight_norms is specified but the network does not support it / scale_weight_norms"
            )
        ...
        if not cache_latents:  # VAEVAE
            vae.requires_grad_(False)
            vae.eval()
            vae.to(accelerator.device, dtype=vae_dtype)

        # ：fp16PyTorchfp16grad scale
        if args.full_fp16:
        ...
        if use_user_config:
            ...
            ), f"There should be a single dataset but {len(train_dataset_group.datasets)} found. This seems to be a bug. / 1{len(train_dataset_group.datasets)}"
        ...
        for epoch in range(num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                with accelerator.accumulate(network):
                    ...
                    loss_weights = batch["loss_weights"]  # sampleweight
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="format to save the model (default is .safetensors) / （safetensors）",
    )

    parser.add_argument("--unet_lr", type=float, default=None, help="learning rate for U-Net / U-Net")
    parser.add_argument("--text_encoder_lr", type=float, default=None, help="learning rate for Text Encoder / Text Encoder")
    ...
    parser.add_argument(
        ...
        type=float,
        default=1,
        help="alpha for LoRA weight scaling, default 1 (same as network_dim for same behavior as old version) / LoRaalpha1（network_dim）",
    )
    parser.add_argument(
        "--network_dropout",
        type=float,
        default=None,
        help="Drops neurons out of training every step (0 or None is default behavior (no dropout), 1 would drop all neurons) / drop（0Nonedropout1dropout）",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--network_train_text_encoder_only", action="store_true", help="only training Text Encoder part / Text Encoder"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no_half_vae",
        action="store_true",
        help="do not use fp16/bf16 VAE in mixed precision (use float VAE) / mixed precision fp16/bf16 VAEfloat VAE",
    )
    return parser
